...
- JS 推送脚本：单文件 `push-docs.mjs`，函数 camelCase。
- API 路径段、库标识：小写连字符。

## 目录与代码结构

- `server/internal/`：按 `api` / `mcp` / `ingest` / `search` 分包；依赖方向 api → ingest/search、mcp → search，禁止 internal 内循环依赖。
- `scripts/push-docs.mjs`：保持零依赖单文件，只用 Node 内置 API（fetch、fs、path），保证复制即跑。
...
- 核心逻辑必须有单测：索引、检索、推送解析、MCP tools 参数组装与响应转换。
- 用 `go test ./...`；测试与被测代码同包。
...
- 推送接口鉴权：`Authorization: Bearer <token>`；读路径（搜索/取文档/列库/MCP）免鉴权。
...
- MCP 层保存文档副本或检索缓存（检索状态只在服务端 SQLite）
